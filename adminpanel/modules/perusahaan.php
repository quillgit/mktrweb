<?php
	function form(){
		...
		if 	(($_GET['mod']=='edit'))
		{
			...
			$edit = mysql_fetch_array(mysql_query("SELECT * FROM tabel_perusahaan WHERE id_perusahaan = '$id'"));
                                    ...
                                    <div class="form-body">
                                        <div class="row">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <a href="admin.php?menu=perusahaan" class="btn btn-outline-warning mr-1 mb-1">Cancel</a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
	...
	if (empty($_GET['mod']))
	{
		...
		{
			$id = isset($_GET['id']) ? $_GET['id']:'';
			$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_perusahaan WHERE id_perusahaan = '$id'"));
			$delete = "DELETE FROM tabel_perusahaan WHERE id_perusahaan ='$id'";				 
			$query = mysql_query($delete);
			if ($query)
			{
				echo "<script>alert('Data Berhasil di Delete');window.location=('admin.php?menu=perusahaan')</script>";
			}
			else
			{
	...
	?>
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Perusaahan</h2>
						<div class="breadcrumb-wrapper col-12">
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="admin.php?menu=home">Home</a>
								</li>
								<li class="breadcrumb-item"><a href="#">Perusaahan</a>
								</li>
								<li class="breadcrumb-item active">Data Perusaahan
								</li>
							</ol>
						</div>
					</div>
		...
		<script type="text/javascript" src="assets/js/menu/data-table-perusahaan.js"></script>
		<!-- Javascript load  -->
	...
	elseif ($_GET['mod']=='new')
	{
		if (empty($_GET['act']))
		{
			...
			$add = "INSERT INTO `tabel_perusahaan` (`title`, `sub_title`, `description`, `sub_title_english`, `description_english`, gambar,  banner, `created`, `author`, `status`, slug) VALUES ('$title', '$sub_title', '$description', '$sub_title_english', '$description_english', '$nama_file_baru', '$fn_mobile_fin', '$date', '$_SESSION[namauser]', '2', '$slug')";
						
			$query = mysql_query($add);
									
			if ($query)
			{
				echo "<script>alert('Data Berhasil di Simpan');window.location=('admin.php?menu=perusahaan')</script>";
			}
			else
			{
	...
	elseif ($_GET['mod']=='edit')
	{
		...
		elseif ($_GET['act']=='process')
		{
			...
			$update = "UPDATE tabel_perusahaan SET  title = '$title',
													sub_title = '$sub_title',
			...
			if ($query)
			{
				...
				if (!empty($lokasifile)){
					...
					if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/png" || $jenis_gambar=="image/gif"){
						...
						$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_perusahaan WHERE id_perusahaan = '$_POST[id]'"));
						if (!empty($gbr['gambar']))  
						{
							if (file_exists("../images/about/$gbr[gambar]"))
							{unlink("../images/about/$gbr[gambar]");}
						}
						mysql_query("UPDATE tabel_perusahaan SET gambar = '$nama_file_baru' WHERE id_perusahaan = '$_POST[id]'");	
					}
				...
				if (!empty($lok_mobile)){
					...
					if($ft_mobile=="image/jpeg" || $ft_mobile=="image/jpg" || $ft_mobile=="image/png" || $ft_mobile=="image/gif"){
						...
						mysql_query("UPDATE tabel_perusahaan SET banner = '$fn_mobile_fin' WHERE id_perusahaan = '$_POST[id]'");	
					}
					
				}
				
				echo "<script>alert('Data Berhasil di Update');window.location=('admin.php?menu=perusahaan')</script>";

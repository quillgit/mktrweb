<?php
	function form(){
		...
		if 	(($_GET['mod']=='edit'))
		{
			...
			$edit = mysql_fetch_array(mysql_query("SELECT * FROM tabel_penghargaan WHERE id_penghargaan = '$id'"));
                                    ...
                                    <div class="form-body">
                                        <div class="row">
											...
											<div class="col-md-12 col-12">
                                                ...
                                                <fieldset class="form-group">
                                                    <div class="custom-file">
														...
														<?php
															if (file_exists("../images/penghargaan/$gambar"))
															{
															echo "<p><img src='../images/penghargaan/$gambar' width='200' border= /></p><br>";
                                            ...
                                            <div class="col-12">
                                                ...
                                                <a href="admin.php?menu=penghargaan" class="btn btn-outline-warning mr-1 mb-1">Cancel</a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
	...
	if (empty($_GET['mod']))
	{
		...
		{
			$id = isset($_GET['id']) ? $_GET['id']:'';
			$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_penghargaan WHERE id_penghargaan = '$id'"));
			$pic = isset($gbr['gambar']) ? $gbr['gambar']:'';
			$delete = "DELETE FROM tabel_penghargaan WHERE id_penghargaan ='$id'";				 
			$query = mysql_query($delete);
			if ($query)
			{
				if (!empty($pic)) 
				{ 
					if (file_exists("../images/penghargaan/$pic"))
					{unlink("../images/penghargaan/$pic");}
				}
				
				
				echo "<script>alert('Data Berhasil di Delete');window.location=('admin.php?menu=penghargaan')</script>";
			}
			else
			{
	...
	?>
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Penghargaan</h2>
						<div class="breadcrumb-wrapper col-12">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active">Data Penghargaan
								</li>
							</ol>
						</div>
					</div>
		...
		<script type="text/javascript" src="assets/js/menu/data-table-penghargaan.js"></script>
		<!-- Javascript load  -->
		...
		if (empty($_GET['act']))
		{
			...
			if (!empty($lokasifile)){
				...
				if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/png" || $jenis_gambar=="image/gif"){
					...
					move_uploaded_file($lokasifile,"../images/penghargaan/$nama_file_baru");
			...
			$add = "INSERT INTO tabel_penghargaan 
			(title, title_english, content, content_english, gambar, link, status, created, author) 
			VALUES 
			('$title', '$title_english', '$content', '$content_english', '$nama_file_baru', '$link', '2', '$date', '$_SESSION[namauser]')";
			...
			if ($query)
			{
				echo "<script>alert('Data Berhasil di Simpan');window.location=('admin.php?menu=penghargaan')</script>";
			}
			else
			{
	...
	elseif ($_GET['mod']=='edit')
	{
		...
		elseif ($_GET['act']=='process')
		{
			...
			$update = "UPDATE tabel_penghargaan SET  title = '$title',
			...
			if ($query)
			{
				if (!empty($lokasifile)){
					...
					if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/png" || $jenis_gambar=="image/gif"){
						...
						$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_penghargaan WHERE id_penghargaan = '$_POST[id]'"));
						if (!empty($gbr['gambar'])) 
						{
							if (file_exists("../images/penghargaan/$gbr[gambar]"))
							{unlink("../images/penghargaan/$gbr[gambar]");}
						}
						mysql_query("UPDATE tabel_penghargaan SET gambar = '$nama_file_baru' WHERE id_penghargaan = '$_POST[id]'");	
					}
					
				}

				echo "<script>alert('Data Berhasil di Update');window.location=('admin.php?menu=penghargaan')</script>";

<?php
	function form(){
		...
		if 	(($_GET['mod']=='edit'))
		{
			...
			$edit = mysql_fetch_array(mysql_query("SELECT * FROM tabel_lowongan_kerja WHERE id_lowongan_kerja = '$id'"));
                                    ...
                                    <div class="form-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <a href="admin.php?menu=lowongan_kerja" class="btn btn-outline-warning mr-1 mb-1">Cancel</a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
	...
	if (empty($_GET['mod']))
	{
		...
		{
			$id = isset($_GET['id']) ? $_GET['id']:'';
			$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_lowongan_kerja WHERE id_lowongan_kerja = '$id'"));
			$pic = isset($gbr['gambar']) ? $gbr['gambar']:'';
			$delete = "DELETE FROM tabel_lowongan_kerja WHERE id_lowongan_kerja ='$id'";				 
			...
			if ($query)
			{
				...
				echo "<script>alert('Data Berhasil di Delete');window.location=('admin.php?menu=lowongan_kerja')</script>";
			}
			else
			{
	...
	?>
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Lowongan Kerja</h2>
						<div class="breadcrumb-wrapper col-12">
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="admin.php?menu=home">Home</a>
								</li>
								<li class="breadcrumb-item"><a href="#">Karier</a>
								</li>
		...
		<script type="text/javascript" src="assets/js/menu/data-table-lowongan-kerja.js"></script>
		<!-- Javascript load  -->
	...
	elseif ($_GET['mod']=='new')
	{
		if (empty($_GET['act']))
		{
			...
			$add = "INSERT INTO tabel_lowongan_kerja 
			(title, title_english, content, content_english, gambar, status, created, author, slug, slug_english, meta_keyword, meta_description) 
			VALUES 
			('$title', '$title_english', '$content', '$content_english', '$nama_file_baru', '2', '$date', '$_SESSION[namauser]', '$slug', '$slug_english', '$meta_keyword', '$meta_description')";
						
			$query = mysql_query($add);
									
			if ($query)
			{
				echo "<script>alert('Data Berhasil di Simpan');window.location=('admin.php?menu=lowongan_kerja')</script>";
			}
			else
			{
	...
	elseif ($_GET['mod']=='edit')
	{
		...
		elseif ($_GET['act']=='process')
		{
			...
			$update = "UPDATE tabel_lowongan_kerja SET  title = '$title',
										...
										meta_description = '$meta_description',
												...
												WHERE id_lowongan_kerja = '$_POST[id]'";
			...
			if ($query)
			{
				if (!empty($lokasifile)){
					...
					if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/png" || $jenis_gambar=="image/gif"){
						...
						$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_lowongan_kerja WHERE id_lowongan_kerja = '$_POST[id]'"));
						if (!empty($gbr['gambar']))  
						{
							if (file_exists("../images/post/$gbr[gambar]"))
							{unlink("../images/post/$gbr[gambar]");}
						}
						mysql_query("UPDATE tabel_lowongan_kerja SET gambar = '$nama_file_baru' WHERE id_lowongan_kerja = '$_POST[id]'");	
					}
					
				}

				echo "<script>alert('Data Berhasil di Update');window.location=('admin.php?menu=lowongan_kerja')</script>";

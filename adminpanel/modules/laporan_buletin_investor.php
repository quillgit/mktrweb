<?php
	function form_2(){
		...
		if 	(($_GET['mod']=='edit_dokumen'))
		{
			...
			$edit = mysql_fetch_array(mysql_query("SELECT * FROM tabel_laporan_buletin_investor WHERE id_laporan_buletin_investor = '$id'"));
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Buletin Investor</h4>
                        </div>
                                    ...
                                    <div class="form-body">
                                        <div class="row">
											...
											<div class="col-md-12 col-12">
                                                ...
                                                <fieldset class="form-group">
                                                    <div class="custom-file">
														...
														<?php
															$editf = mysql_fetch_array(mysql_query("SELECT file_dokumen FROM tabel_laporan_buletin_investor WHERE id_laporan_buletin_investor = '$id'"));
                                            ...
                                            <div class="col-12">
                                                ...
                                                <a href="admin.php?menu=laporan_buletin_investor" class="btn btn-outline-warning mr-1 mb-1">Cancel</a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
	...
	if (empty($_GET['mod']))
	{
		...
		{
			$id = isset($_GET['id']) ? $_GET['id']:'';
			$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_laporan_buletin_investor WHERE id_laporan_buletin_investor = '$id'"));
			$pic = isset($gbr['file_dokumen']) ? $gbr['file_dokumen']:'';
			$delete = "DELETE FROM tabel_laporan_buletin_investor WHERE id_laporan_buletin_investor ='$id'";				 
			...
			if ($query)
			{
				...
				echo "<script>alert('Data Berhasil di Delete');window.location=('admin.php?menu=laporan_buletin_investor')</script>";
			}
			else
			{
		...
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Buletin Investor</h2>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="table-responsive">
					<table class="table data-thumb-view" id="table-product">
						...
						<tbody id="product-list">
							<?php
							$no = 1;
							$banner = mysql_query("SELECT * FROM tabel_laporan_buletin_investor ORDER BY created DESC");
							...
							<tr>
								...
								<td>$list[created]</td>
								<td align='center'><a href='?menu=$_GET[menu]&mod=edit_dokumen&id=$list[id_laporan_buletin_investor]'><img src=img/edit.png border=0 /></a> ";
							?>
								<a href="?menu=<?php echo "$_GET[menu]"; ?>&act=delete&id=<?php echo "$list[id_laporan_buletin_investor]"; ?>" 
	...
	elseif ($_GET['mod']=='new')
	{
		if (empty($_GET['act']))
		{
			...
			$add = "INSERT INTO tabel_laporan_buletin_investor 
			...
			if ($query)
			{
				echo "<script>alert('Data Berhasil di Simpan');window.location=('admin.php?menu=laporan_buletin_investor')</script>";
			}
			else
			{
	...
	elseif ($_GET['mod']=='edit_dokumen')
	{
		...
		elseif ($_GET['act']=='process')
		{
			...
			$update = "UPDATE tabel_laporan_buletin_investor SET  title = '$title',
													title_english = '$title_english',
													laporan_date = '$newlaporan_date',
												 updated = '$date',
												 updater = '$_SESSION[namauser]'
												WHERE id_laporan_buletin_investor = '$_POST[id]'";
			...
			if ($query)
			{
				...
				if (!empty($lokasifile_f)){
						...
						if($jenis_file=="application/pdf" || $jenis_file=="application/msword" || $jenis_file=="application/vnd.openxmlformats-officedocument.wordprocessingml.document" || $jenis_file=="application/excel" || $jenis_file=="image/jpeg" || $jenis_file=="image/jpg" || $jenis_file=="image/png" || $jenis_file=="image/gif"){
							...
							$fly = mysql_fetch_array (mysql_query("SELECT file_dokumen FROM tabel_laporan_buletin_investor WHERE id_laporan_buletin_investor = '$_POST[id]'"));
							if (!empty($fly['file_dokumen'])) 
							{
								if (file_exists("../dokumen/$fly[file_dokumen]"))
								{unlink("../dokumen/$fly[file_dokumen]");}
							}
							mysql_query("UPDATE tabel_laporan_buletin_investor SET file_dokumen = '$nama_file_f' WHERE id_laporan_buletin_investor = '$_POST[id]'");	
						}
						
					}
				
				echo "<script>alert('Data Berhasil di Update');window.location=('admin.php?menu=laporan_buletin_investor')</script>";

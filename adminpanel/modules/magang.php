<?php
	function form(){
		...
		if 	(($_GET['mod']=='edit'))
		{
			...
			$edit = mysql_fetch_array(mysql_query("SELECT * FROM tabel_magang WHERE id_magang = '$id'"));
                                    ...
                                    <div class="form-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <a href="admin.php?menu=magang" class="btn btn-outline-warning mr-1 mb-1">Cancel</a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
	...
	if (empty($_GET['mod']))
	{
		...
		{
			$id = isset($_GET['id']) ? $_GET['id']:'';
			$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_magang WHERE id_magang = '$id'"));
			$pic = isset($gbr['gambar']) ? $gbr['gambar']:'';
			$delete = "DELETE FROM tabel_magang WHERE id_magang ='$id'";				 
			...
			if ($query)
			{
				...
				echo "<script>alert('Data Berhasil di Delete');window.location=('admin.php?menu=magang')</script>";
			}
			else
			{
	...
	?>
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Magang</h2>
						<div class="breadcrumb-wrapper col-12">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active">Magang
								</li>
							</ol>
						</div>
					</div>
		...
		<script type="text/javascript" src="assets/js/menu/data-table-magang.js"></script>
		<!-- Javascript load  -->
	...
	elseif ($_GET['mod']=='new')
	{
		if (empty($_GET['act']))
		{
			...
			if ($query)
			{
				echo "<script>alert('Data Berhasil di Simpan');window.location=('admin.php?menu=magang')</script>";
			}
			else
			{
	...
	elseif ($_GET['mod']=='edit')
	{
		...
		elseif ($_GET['act']=='process')
		{
			...
			$update = "UPDATE tabel_magang SET  title = '$title',
			...
			if ($query)
			{
				if (!empty($lokasifile)){
					...
					if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/png" || $jenis_gambar=="image/gif"){
						...
						$gbr = mysql_fetch_array (mysql_query("SELECT * FROM tabel_magang WHERE id_magang = '$_POST[id]'"));
						if (!empty($gbr['gambar']))  
						{
							if (file_exists("../images/post/$gbr[gambar]"))
							{unlink("../images/post/$gbr[gambar]");}
						}
						mysql_query("UPDATE tabel_magang SET gambar = '$nama_file_baru' WHERE id_magang = '$_POST[id]'");	
					}
					
				}

				echo "<script>alert('Data Berhasil di Update');window.location=('admin.php?menu=magang')</script>";

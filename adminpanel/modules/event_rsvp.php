<?php
	function form(){
		...
		if 	(($_GET['mod']=='edit'))
		{
			...
			$rsvp_content 		= stripslashes($edit['rsvp_content']);
			$is_rsvp 		    = stripslashes($edit['is_rsvp']);
		...
		{ 
			...
			$rsvp_content 		= isset($_POST['rsvp_content']) ? $_POST['rsvp_content']:''; 
			$is_rsvp 		    = isset($_POST['is_rsvp']) ? $_POST['is_rsvp']:''; 
	...
	?>
                                    ...
                                    <div class="form-body">
                                        <div class="row">
                                            ...
                                            <div class="col-md-12 col-12 mb-2">
												<h6>Reservasi</h6>
                                                <div class="col-sm-10 set-padding">								
                    								<div class="form-check form-check-inline">
                    									<input class="form-check-input" type="radio" name="is_rsvp" id="Y" value="Y" <?php echo ($is_rsvp=='Y') ? 'checked="checked"':'';?>>
                    									<label class="form-check-label" for="Y">Ya</label>
                    								</div>
                    								<div class="form-check form-check-inline">
                    									<input class="form-check-input" type="radio" name="is_rsvp" id="N" value="N" <?php echo ($is_rsvp=='N') ? 'checked="checked"':'';?>>
                                            ...
                                            <div class="col-md-2 col-2 mb-2">
												<h6>Reservasi</h6>
												<select class="form-control" name="is_rsvp" onchange='CheckRsvp(this.value);'>
                                                    <option value="N" <?php echo ($is_rsvp=='N') ? 'selected="selected"':'';?>>Tidak</option>
                    								<option value="Y" <?php echo ($is_rsvp=='Y') ? 'selected="selected"':'';?>>Ya</option>
                                                </select>
                                            </div>
											...
											<div class="col-md-12 col-12 rsvp-content" id="rsvp-content" style='<?php echo $disp_block;?>'>
												<h6>Reservasi Content</h6>
                                                <div class="form-label-group">
													<textarea class="form-control" rows="3" name="rsvp_content" placeholder="Reservasi Content ..."><?php echo"$rsvp_content"; ?></textarea>
                                                </div>
                                            </div>
											
											<div class="col-md-12 col-12">
												<h6>Gambar <b>(800px x 400px)</b></h6>
        ...
        <script>
            function CheckRsvp(val) {
    			var element = document.getElementById('rsvp-content');
    			if (val == 'Y')
	...
	?>
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Event Reservation</h2>
						<div class="breadcrumb-wrapper col-12">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active">Data Event Reservation
								</li>
							</ol>
						</div>
					</div>
		...
		<script type="text/javascript" src="assets/js/menu/data-table-event-rsvp.js"></script>
		<!-- Javascript load  -->
	...
	elseif ($_GET['mod']=='new')
	{
		if (empty($_GET['act']))
		{
			...
			$rsvp_content 	 	= mysql_real_escape_string(str_replace("'", "&#39;", $_POST['rsvp_content']));
			$is_rsvp 	 		= mysql_real_escape_string(str_replace("'", "&#39;", $_POST['is_rsvp']));
			//cek SLUG
			...
			(title, content, rsvp_content, is_rsvp, gambar, start_date, end_date, durasi, maps, lokasi, kontak, status, created, author, slug, meta_keyword, meta_description) 
			VALUES 
			('$title', '$content', '$rsvp_content', '$is_rsvp', '$nama_file_baru', '$newStartDate', '$newEndDate', '$durasi', '$maps', '$lokasi', '$kontak', '2', '$date', '$_SESSION[namauser]', '$slug', '$meta_keyword', '$meta_description')";

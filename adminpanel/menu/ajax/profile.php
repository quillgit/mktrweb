...
if($_GET) {
    ...
    if(isset($search)) {
		...
		*
		FROM tabel_about_us 
		WHERE tabel_about_us.title LIKE '%$search%' ORDER BY tabel_about_us.id_about_us ASC LIMIT $length OFFSET $start");
    } else {
        $supplier = mysql_query("SELECT 
		*
		FROM tabel_about_us 
    ...
    $records_total = mysql_query("SELECT * FROM tabel_about_us");
    $data = new stdClass();
    $data->draw = $draw;
    ...
    while ($result_produk = mysql_fetch_array($supplier)) 
    {
        ...
        $data_produk[4] = "<a href='?menu=profile&mod=edit&id=$result_produk[id_about_us]'><i class='feather icon-edit'></i></a>";

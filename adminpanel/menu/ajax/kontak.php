...
if($_GET) {
    ...
    if(isset($search)) {
		...
		*
		FROM tabel_about_us 
		WHERE tabel_about_us.title LIKE '%$search%' AND id_about_us = '2' ORDER BY tabel_about_us.updated DESC LIMIT $length OFFSET $start");
    } else {
        $supplier = mysql_query("SELECT 
		*
		FROM tabel_about_us 	
    ...
    $records_total = mysql_query("SELECT * FROM tabel_about_us WHERE id_about_us = '2'");
    $data = new stdClass();
    $data->draw = $draw;
    ...
    while ($result_produk = mysql_fetch_array($supplier)) 
    {
        ...
        $data_produk[5] = "<a href='?menu=kontak&mod=edit&id=$result_produk[id_about_us]'><i class='feather icon-edit'></i></a>";
